...
4. **Configure Environment Variables**
   ...
   - **Delete Unnecessary Variables:** If you have any unnecessary environment variables, you can delete them by clicking the **Delete (trashcan)** button next to the variable. One example of an unnecessary variable is `ALPACA_API_KEY` which is only needed for Alpaca live trading.
...
.. list-table:: General Environment Variables
   ...
   * - MARKET
     ...
     - NYSE
   * - TRADING_BROKER
     - **(Optional)** For live trading, specify the broker to use for executing trades. Valid options (case insensitive): "alpaca", "ib" (or "interactivebrokers"), "ibrest" (or "interactivebrokersrest")
     - alpaca
   * - DATA_SOURCE
     - **(Optional)** For live trading, specify a separate data source for market data. Valid options (case insensitive): "alpaca", "yahoo", "ibkr", "interactivebrokersrest"
     - yahoo
   * - DATA_SOURCE_DELAY
