...
class Data:
    ...
    def __init__(
        ...
        if isinstance(self.df.index, pd.MultiIndex):
            start_ts = self.df.index.get_level_values("timestamp")[0]
            end_ts = self.df.index.get_level_values("timestamp")[-1]
        else:
        ...
        # PERF: Cache bars schema for get_bars(); lazily populate _bars_df in repair_times_and_fill.
    ...
    def repair_times_and_fill(self, idx):
        ...
        if has_dups is None:
            has_dups = self.df.index.duplicated().any()
        if has_dups:
            self.df = self.df[~self.df.index.duplicated(keep="first")]

        # Handle MultiIndex: build new_index for reindexing; single index uses idx directly.
        if isinstance(self.df.index, pd.MultiIndex):
            new_index = pd.MultiIndex.from_arrays(
                [self.df.index.get_level_values(level) for level in self.df.index.names if level != "timestamp"]
                + [idx],
        ...
        # CRITICAL FIX (upstream): Time-gap aware forward-fill for bid/ask columns.
        # Prevent stale weekend/after-hours quote data from being forward-filled into the first bar
        # of a new trading session.
        ...
        # Use timestamp level for MultiIndex, else index directly.
        index_for_time = df.index.get_level_values("timestamp") if isinstance(df.index, pd.MultiIndex) else df.index

        if (
            apply_quote_session_boundaries
            and quote_cols_present
            and isinstance(index_for_time, pd.DatetimeIndex)
        ):
            time_diff = pd.Series(index_for_time).diff()
            max_gap_minutes = 120
            ...
            if session_boundaries.sum() > 0:
                for col in quote_cols_present:
                    if col in self.df.columns:
                        ...
                        clear_mask = pd.Series(
                            session_boundaries.values & original_values.isna().values,
                            index=df.index,
                        )
                        if clear_mask.any():
                            df.loc[clear_mask, col] = float("nan")

        # Forward fill: exclude OHLC and quote cols from standard ffill; handle quotes separately.
        ohlc_cols = ["open", "high", "low"]
        quote_cols_set = set(quote_cols)
        non_ohlc_non_quote = [c for c in df.columns if c not in ohlc_cols and c not in quote_cols_set]
        if non_ohlc_non_quote:
            df[non_ohlc_non_quote] = df[non_ohlc_non_quote].ffill()

        # Segment-wise ffill for quote columns (don't fill across session boundaries)
        if (
            apply_quote_session_boundaries
            and quote_cols_present
            and isinstance(index_for_time, pd.DatetimeIndex)
            ...
            time_diff = pd.Series(index_for_time).diff()
            session_boundaries = time_diff > pd.Timedelta(minutes=120)
            segment_ids = pd.Series(session_boundaries.cumsum().values, index=df.index)
            for col in quote_cols_present:
        ...
        # Vectorized NaN filling for OHLC columns
        ...
        # PERF (upstream): Precompute price_change, dividend_yield, return once per dataset.
        try:
            ...
            elif "return" not in df.columns and "close" in df.columns:
                close_series = df["close"]
                try:
                    close = close_series.to_numpy(dtype="float64", copy=False)
                except Exception:
